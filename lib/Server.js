...
class Server {
  ...
  constructor(options) {
    ...
    if (pm2) {
      pm2.list((error, list) => {
        ...
        if (list.length) {
          this.pm2_ids = list.map(item => item.pm2_env.pm_id);
  ...
  async [PRIVATE.handleRequest](request, response) {
    ...
    if (incoming.method === 'DELETE') {
      ...
      if (pm2) {
        const ids = this.pm2_ids;
        for (let i = 0; i < ids.length; i += 1) {
          if (Number(process.env.pm_id) !== Number(ids[i])) {
            pm2.sendDataToProcessId({
              ...
              if (error) {
                console.log('send pm2 IPC message error => ', error);
              }
            });
          }
        }
