...
const PRIVATE_METHODS = [
  'checkMethod', 'handleRequest', 'getRequestDigest', 'willApplyCache', 'willPurgeCache', 'useCachePolicy'
];
const PROTECT_METHODS = ['listen'];
const VIRTUAL_METHODS = [
  'reviseRequest', 'inferUpstream', 'mustSkipCache', 'mustWaitAgent', 'canCacheError', 'getCachePolicy'
];
...
class Server {
  ...
  constructor(options = {}) {
    ...
    this.bridge = new Bridge({
      'purge-cache': (payload) => this.purgeCache(payload.url)
    });
  ...
  async [PRIVATE.handleRequest](request, response) {
    ...
    if (incoming.method === 'DELETE') {
      ...
      this.bridge.sendMessage('purgeCache', { url: incoming.url });
      handler.respond(200, `${cacheKeys.join(',')} purged`);
      return;
    }

    // skip non GET requests
    if (incoming.method !== 'GET') {
      handler.respond(403, 'not implemented');
      return;
    }
    ...
    if (!thread) {
      ...
      if (!upstream) {
        handler.respond(500, 'upstream not provided');
        return;
      }
